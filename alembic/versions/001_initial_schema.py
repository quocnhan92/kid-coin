...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column('category', sa.Enum('Học tập', 'Việc nhà', 'Giải trí', 'Xã hội', 'Cá nhân', 'Kiếm tiền', 'Khác', name='category', create_constraint=False), nullable=False),
        sa.Column('verification_type', sa.Enum('Tự động duyệt', 'Cần chụp ảnh', 'Bố mẹ kiểm tra trực tiếp', name='verificationtype', create_constraint=False), server_default='Cần chụp ảnh'),
    )
    ...
    op.create_table(
        ...
        sa.Column('role', sa.Enum('PARENT', 'KID', name='role', create_constraint=False), nullable=False),
    ...
    op.create_table(
        ...
        sa.Column('category', sa.Enum('Học tập', 'Việc nhà', 'Giải trí', 'Xã hội', 'Cá nhân', 'Kiếm tiền', 'Khác', name='category', create_constraint=False), server_default='Khác'),
        sa.Column('verification_type', sa.Enum('Tự động duyệt', 'Cần chụp ảnh', 'Bố mẹ kiểm tra trực tiếp', name='verificationtype', create_constraint=False), server_default='Cần chụp ảnh'),
    ...
    op.create_table(
        ...
        sa.Column('role', sa.Enum('ADMIN', 'MEMBER', name='clubrole', create_constraint=False), server_default='MEMBER', nullable=False),
    ...
    op.create_table(
        ...
        sa.Column('status', sa.Enum('PENDING', 'ACCEPTED', 'REJECTED', name='invitationstatus', create_constraint=False), server_default='PENDING', nullable=False),
        sa.Column('created_at', sa.DateTime(timezone=True), server_default=sa.text('now()')),
        sa.Column('updated_at', sa.DateTime(timezone=True), nullable=True),
    ...
    op.create_table(
        ...
        sa.Column('status', sa.Enum('PENDING_APPROVAL', 'APPROVED', 'REJECTED', name='taskstatus', create_constraint=False), server_default='PENDING_APPROVAL'),
    ...
    op.create_table(
        ...
        sa.Column('status', sa.Enum('PENDING_DELIVERY', 'DELIVERED', name='redemptionstatus', create_constraint=False), server_default='PENDING_DELIVERY'),
        sa.Column('created_at', sa.DateTime(timezone=True), server_default=sa.text('now()')),
    ...
    op.create_table(
        ...
        sa.Column('transaction_type', sa.Enum('TASK_COMPLETION', 'REWARD_REDEMPTION', 'PENALTY', 'BONUS', name='transactiontype', create_constraint=False), nullable=False),
    ...
    op.create_table(
        ...
        sa.Column('type', sa.Enum('SYSTEM', 'CLUB_INVITE', 'KID_CLUB_INVITE', 'TASK_ASSIGNED', name='notificationtype', create_constraint=False), server_default='SYSTEM', nullable=False),
    ...
    op.create_table(
        ...
        sa.Column('status', sa.Enum('INIT', 'PROCESSING', 'SUCCESS', 'FAILED', name='auditstatus', create_constraint=False), server_default='INIT', nullable=False, index=True),
